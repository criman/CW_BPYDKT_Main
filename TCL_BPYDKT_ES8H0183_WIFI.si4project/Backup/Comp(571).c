...
void    Log_Comp(void)
{
	if (Comp.f_DrvOn == OFF)
	{
		...
		{		
			...
			if ((Comp.f_AppOn == ON) && (Comp.u16_RestartDelay == 0) && (Comp.u8_SelTestDelay == 0) && (Comp.u8_PowerOnDelay == 0) && (WaterBox.f_Full == 0))
			{
				//启动刹那
	...
	else
	{
		if (((Comp.f_AppOn == OFF) && (Comp.u16_RunMinDelay == 0)) || WaterBox.f_Full || 		 			
			...
			Fan.Indoor.f_Error || Fan.Outdoor.f_Error) && (System.Enum_Status != ENUM_STATUS_BUSSCHK)))	
		{
			//关停刹那
