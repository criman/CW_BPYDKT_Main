...
void    Uart3_Config_lhh(void)
{
    ...
    GPIO_Init(GPIOA, GPIO_Pin_5, &y);               //PA5---TxD
    ...
    y.GPIO_OD = GPIO_ODE_Output_Disable;
    y.GPIO_DS = GPIO_DS_Output_Strong;
    GPIO_Init(GPIOA, GPIO_Pin_4, &y);               //PA4---RxD
    ...
    UART2_TxEnable();                               //UART3发送使能
    UART2_RxEnable();								//UART3接收使能 
}
...
void send_Three_Data(void)
{
    ...
    g_txbuf[5] = 0;    //内机故障码1
		...
		*/
		...
		g_txbuf[5] |= (unsigned char)System.f_InEEpromErr<<1;
		g_txbuf[5] |= (unsigned char)F_uart0_CommErr<<2;
		g_txbuf[5] |= (unsigned char)T1.f_Error<<3;
		g_txbuf[5] |= (unsigned char)T2.f_Error<<4;
		g_txbuf[5] |= (unsigned char)T3.f_Error<<5;
		g_txbuf[5] |= (unsigned char)T4.f_Error<<6;
		g_txbuf[5] |= (unsigned char)Tp.f_Error<<7;
...
//		g_txbuf[6] |= (unsigned char)System.f_InEEpromErr<<4;
    ...
    g_txbuf[7] = 0x00;//内机保护标志
		...
		*/				
		g_txbuf[7] |= (unsigned char)Protect.f_Tp;
		...
		g_txbuf[7] |= (unsigned char)Protect.f_HumiT1Low<<6;
...
//		g_txbuf[11] = 0x00;  //外机故障码3
		...
		g_txbuf[9]  = g_tx_OutDoorStatus1;		//外机故障码1
		g_txbuf[10] = g_tx_OutDoorStatus2;		//外机故障码2
		g_txbuf[11] = g_tx_OutDoorStatus3;		//外机故障码3
				
    g_txbuf[12] = 0x00; //2	//外机保护标志1
    g_txbuf[13] = 0x00;//128	//外机保护标志2
				
//    g_txbuf[14] = T1.s8_Value + 40; // T=25-40=-15 室内温度T+40
	g_txbuf[14] = (unsigned char)(T1.s16_ValueMul10 + 400); // T=25-40=-15 室内温度T+40		
//    g_txbuf[15] =  T2.s8_Value + 40; // T=26-40=-14 室内盘管温度T+40
	g_txbuf[15] = (unsigned char)(T2.s16_ValueMul10 + 400); // T=25-40=-15 室内温度T+40

    g_txbuf[16] = 0x00;//82	//室内湿度值
			
//    g_txbuf[17] = T3.s8_Value + 40;//室外盘管温度16-40=-24		
    g_txbuf[17] = (unsigned char)(T3.s16_ValueMul10 + 400); // T=25-40=-15 室内温度T+40
//    g_txbuf[18] = T4.s8_Value + 40;//室外环境温度30-40=-10
    g_txbuf[18] = (unsigned char)(T4.s16_ValueMul10 + 400); // T=25-40=-15 室内温度T+40
//    g_txbuf[19] = Tp.s8_Value + 40; //1,室外排气温度=1-40=-39
	g_txbuf[19] = (unsigned char)(Tp.s16_ValueMul10 + 400); // T=25-40=-15 室内温度T+40
					
    g_txbuf[20] = System.u8_IPMTemp;    //IPM温度
    ...
    g_txbuf[25] = Tempr.T1TSDeltaZone * 10;//电子膨胀阀开度
    g_txbuf[26] = SystemMode.f_RoughCon *100;	
...
//    g_txbuf[31] = ;	//母线电压
//    g_txbuf[32] = 0x00;//
    g_txbuf[31] = (unsigned char)(System.u16_DCVoltValue & 0x00ff);//母线电压
    g_txbuf[32] = (unsigned char)((System.u16_DCVoltValue & 0xff00)>>8);	// 
    ...
    g_txbuf[35] = System.u8_OutDoorCurrentMul8;	//整机电流
    g_txbuf[36] = 0x00; //3200
                
    g_txbuf[37] = (unsigned char)(System.u16_ACVoltValue & 0x00ff);//交流电压
    g_txbuf[38] = (unsigned char)((System.u16_ACVoltValue & 0xff00)>>8);	//
    ...
    g_txbuf[43] = (unsigned char)((T1.s16_ValueMul10 + 400) >>8); 		//预留1	
    g_txbuf[44] = (unsigned char)((T2.s16_ValueMul10 + 400) >>8);	
                
    g_txbuf[45] = (unsigned char)((T3.s16_ValueMul10 + 400) >>8);		//预留2
    g_txbuf[46] = (unsigned char)((T4.s16_ValueMul10 + 400) >>8);	
                
    g_txbuf[47] = (unsigned char)((Tp.s16_ValueMul10 + 400) >>8);//预留3			
    g_txbuf[48] = 0;//
    ...
    UART_ITConfig(UART2, UART_IT_TB, ENUM_ENABLE);    //??????,?????????????
}
...
//    U8 checksum;
    ...
    U8_Uart0_AppTxBuf[2]  = C_COMMDISP_TX_BYTE3;    	//数据长度      //发送字节数据
    U8_Uart0_AppTxBuf[3]  = CommDisp.Tx.Byte4.Byte;                     //发送字节数据
    U8_Uart0_AppTxBuf[4]  = CommDisp.Tx.Byte5.Byte;                     //发送字节数据
    U8_Uart0_AppTxBuf[5]  = CommDisp.Tx.Byte6.Byte;                     //发送字节数据
    U8_Uart0_AppTxBuf[6]  = CommDisp.Tx.Byte7.Byte;                     //发送字节数据
    U8_Uart0_AppTxBuf[7]  = CommDisp.Tx.Byte8.Byte;                     //发送字节数据
    U8_Uart0_AppTxBuf[8]  = CommDisp.Tx.Byte9.Byte;                     //发送字节数据
    U8_Uart0_AppTxBuf[9]  = CommDisp.Tx.Byte10.Byte;                    //发送字节数据
    U8_Uart0_AppTxBuf[10] = CommDisp.Tx.Byte11.Byte;                    //发送字节数据
    U8_Uart0_AppTxBuf[11] = CommDisp.Tx.Byte12.Byte;                    //发送字节数据
    U8_Uart0_AppTxBuf[12] = CommDisp.Tx.Byte13.Byte;                    //发送字节数据
    U8_Uart0_AppTxBuf[13] = CommDisp.Tx.Byte14.Byte;                    //发送字节数据
    U8_Uart0_AppTxBuf[14] = CommDisp.Tx.Byte15.Byte;                    //发送字节数据
    U8_Uart0_AppTxBuf[15] = CommDisp.Tx.Byte16.Byte;                    //发送字节数据
    U8_Uart0_AppTxBuf[16] = CommDisp.Tx.Byte17.Byte;                    //发送字节数据
    U8_Uart0_AppTxBuf[17] = CommDisp.Tx.Byte18.Byte;                    //发送字节数据
    U8_Uart0_AppTxBuf[18] = CommDisp.Tx.Byte19.Byte;                    //发送字节数据
    U8_Uart0_AppTxBuf[19] = CommDisp.Tx.Byte20.Byte;                    //发送字节数据
    U8_Uart0_AppTxBuf[20] = CommDisp.Tx.Byte21.Byte;                    //发送字节数据
    U8_Uart0_AppTxBuf[21] = CommDisp.Tx.Byte22.Byte;                    //发送字节数据
    U8_Uart0_AppTxBuf[22] = CommDisp.Tx.Byte23.Byte;                    //发送字节数据
    U8_Uart0_AppTxBuf[23] = CommDisp.Tx.Byte24.Byte;                    //发送字节数据
    U8_Uart0_AppTxBuf[24] = CommDisp.Tx.Byte25.Byte;                    //发送字节数据
    U8_Uart0_AppTxBuf[25] = CommDisp.Tx.Byte26.Byte;                    //发送字节数据
    U8_Uart0_AppTxBuf[26] = CommDisp.Tx.Byte27.Byte;                    //发送字节数据

   
    U8_Uart0_AppTxBuf[UART0_TX_CHKSUM] = CommDisp_Check8(U8_Uart0_AppTxBuf, (UART0_TX_LEN-1));   //校验
    
//    checksum = 0;
//    
//    for (i=0; i<UART0_TX_CHKSUM; i++)
//    {
...
//    checksum += 1;
//    
//    U8_Uart0_AppTxBuf[UART0_TX_CHKSUM] = checksum;                      //校验
...
//        chksum += 1;
		...
		chksum = CommDisp_Check8(U8_Uart0_DrvRxBuf, (UART0_RX_LEN-1));
              
        if ((U8_Uart0_DrvRxBuf[UART0_RX_HEAD_FIRST] == C_UART0_RX_HEAD_FIRST) && (U8_Uart0_DrvRxBuf[UART0_RX_HEAD_SECOND] == C_UART0_RX_HEAD_SECOND))   //头码正确
        {
            if (U8_Uart0_DrvRxBuf[UART0_RX_CHKSUM] == chksum)              //检验通过，指令处理
            {
                ...
                //数据接收分析
                if (U8_Uart0_RxBuf[2] == C_COMMDISP_RX_BYTE3)             //接收字节长度一致
				{
					CommDisp.Rx.Byte4.Byte  = U8_Uart0_RxBuf[3];          //接收数据
					CommDisp.Rx.Byte5.Byte  = U8_Uart0_RxBuf[4];          //接收数据
					CommDisp.Rx.Byte6.Byte  = U8_Uart0_RxBuf[5];          //接收数据
					CommDisp.Rx.Byte7.Byte  = U8_Uart0_RxBuf[6];          //接收数据
					CommDisp.Rx.Byte8.Byte  = U8_Uart0_RxBuf[7];          //接收数据
					CommDisp.Rx.Byte9.Byte  = U8_Uart0_RxBuf[8];          //接收数据
					CommDisp.Rx.Byte10.Byte = U8_Uart0_RxBuf[9];          //接收数据
					CommDisp.Rx.Byte11.Byte = U8_Uart0_RxBuf[10];         //接收数据					
					CommDisp.Rx.Byte12.Byte = U8_Uart0_RxBuf[11];		  //接收数据
					CommDisp.Rx.Byte13.Byte = U8_Uart0_RxBuf[12];		  //接收数据
					CommDisp.Rx.Byte14.Byte = U8_Uart0_RxBuf[13];		  //接收数据
					CommDisp.Rx.Byte15.Byte = U8_Uart0_RxBuf[14];		  //接收数据
...
void    Uart1_Config(void)
{
    ...
    y.GPIO_OD = GPIO_ODE_Output_Disable;
    y.GPIO_DS = GPIO_DS_Output_Strong;
    GPIO_Init(GPIOA, GPIO_Pin_17, &y);               //PA17---TxD
    ...
    y.GPIO_OD = GPIO_ODE_Output_Disable;
    y.GPIO_DS = GPIO_DS_Output_Strong;
    GPIO_Init(GPIOA, GPIO_Pin_18, &y);               //PA18---RxD
...
void    Uart2_Config(void)
{
    ...
    uart.UART_StopBits = UART_StopBits_1;          //停止位个数:1
    uart.UART_TxMode = UART_DataMode_8Even;            //发送数据格式:8位数据偶校验
    uart.UART_TxPolar = UART_Polar_Normal;         //发送端口极性:正常
    uart.UART_RxMode = UART_DataMode_8Even;            //接收数据格式:8位数据偶校验
    uart.UART_RxPolar = UART_Polar_Normal;         //接收端口极性:正常
    uart.UART_BaudRate = 9600;                     //波特率
    uart.UART_ClockSet = UART_Clock_1;             //时钟选择:Pclk
    UART_Init(UART2, &uart);
...
void    Uart3_Config(void)
{
    ...
    y.GPIO_OD = GPIO_ODE_Output_Disable;
    y.GPIO_DS = GPIO_DS_Output_Strong;
    GPIO_Init(GPIOA, GPIO_Pin_26, &y);               //PA26---TxD
    ...
    y.GPIO_OD = GPIO_ODE_Output_Disable;
    y.GPIO_DS = GPIO_DS_Output_Strong;
    GPIO_Init(GPIOA, GPIO_Pin_27, &y);               //PA27---RxD
...
void    Uart4_Config(void)
{
    ...
    y.GPIO_OD = GPIO_ODE_Output_Disable;
    y.GPIO_DS = GPIO_DS_Output_Strong;
    GPIO_Init(GPIOB, GPIO_Pin_11, &y);               //PB11---TxD
    ...
    y.GPIO_OD = GPIO_ODE_Output_Disable;
    y.GPIO_DS = GPIO_DS_Output_Strong;
    GPIO_Init(GPIOB, GPIO_Pin_10, &y);               //PB10---RxD

    uart.UART_StopBits = UART_StopBits_2;          //停止位个数:2
    uart.UART_TxMode = UART_DataMode_8Even;        //发送数据格式:8位数据+偶校验
    uart.UART_TxPolar = UART_Polar_Normal;         //发送端口极性:正常
    uart.UART_RxMode = UART_DataMode_8Even;        //接收数据格式:8位数据+偶校验
    uart.UART_RxPolar = UART_Polar_Normal;         //接收端口极性:正常
    uart.UART_BaudRate = 1200;                     //波特率
    uart.UART_ClockSet = UART_Clock_1;             //时钟选择:Pclk
...
//    U8_Uart4_AppTxBuf[UART4_TX_HEAD_SECOND]  = C_UART4_TX_HEAD_SECOND;   //头码2		
	...
	U8_Uart4_AppTxBuf[1]  = C_COMMMOTDRV_TX_SRCADDR; //通讯源地址       //发送字节数据
    U8_Uart4_AppTxBuf[2]  = C_COMMMOTDRV_TX_TAGADDR; //通讯目标地址     //发送字节数据
    U8_Uart4_AppTxBuf[3]  = CommMotDrv.u8_TXCommand; //运行命令         //发送字节数据
    U8_Uart4_AppTxBuf[4]  = C_COMMMOTDRV_LENTH;      //参数长度         //发送字节数据
    U8_Uart4_AppTxBuf[5]  = CommMotDrv.TxBuf.u8_Byte5;                  //发送字节数据
    U8_Uart4_AppTxBuf[6]  = CommMotDrv.TxBuf.u8_Byte6;                  //发送字节数据
    U8_Uart4_AppTxBuf[7]  = CommMotDrv.TxBuf.u8_Byte7;                  //发送字节数据
    U8_Uart4_AppTxBuf[8]  = CommMotDrv.TxBuf.u8_Byte8;                  //发送字节数据
    U8_Uart4_AppTxBuf[9]  = CommMotDrv.TxBuf.u8_Byte9;                  //发送字节数据
    U8_Uart4_AppTxBuf[10] = CommMotDrv.TxBuf.u8_Byte10;                 //发送字节数据
 	U8_Uart4_AppTxBuf[11] = CommMotDrv.TxBuf.u8_Byte11;					//发送字节数据
    U8_Uart4_AppTxBuf[12] = CommMotDrv.TxBuf.u8_Byte12;                 //发送字节数据
    U8_Uart4_AppTxBuf[13] = CommMotDrv.TxBuf.u8_Byte13;                 //发送字节数据
    U8_Uart4_AppTxBuf[14] = CommMotDrv.TxBuf.u8_Byte14;                 //发送字节数据
    U8_Uart4_AppTxBuf[15] = CommMotDrv.TxBuf.u8_Byte15;                 //发送字节数据
    U8_Uart4_AppTxBuf[16] = CommMotDrv.TxBuf.u8_Byte16;                 //发送字节数据
    U8_Uart4_AppTxBuf[17] = CommMotDrv.TxBuf.u8_Byte17; 
    ...
    U8_Uart4_AppTxBuf[UART4_TX_CHKSUML] = checksum & 0xFF;                //校验-低位
	U8_Uart4_AppTxBuf[UART4_TX_CHKSUMH] = checksum >> 8;    
...
void    App_Uart4_Read_Frame(void)
{
    ...
    {        
        ...
        if (U8_Uart4_DrvRxBuf[UART4_RX_HEAD_FIRST] == C_UART4_RX_HEAD_FIRST)   	//头码正确
        {
            if ((U8_Uart4_DrvRxBuf[UART4_RX_CHKSUML] == ChkSumL) && (U8_Uart4_DrvRxBuf[UART4_RX_CHKSUMH] == ChkSumH))             //检验通过，指令处理
            {
                ...
                //数据接收分析
				if ((U8_Uart4_RxBuf[1] == C_COMMMOTDRV_RX_SRCADDR) && (U8_Uart4_RxBuf[2] == C_COMMMOTDRV_RX_TAGADDR) && (U8_Uart4_RxBuf[4] == C_COMMMOTDRV_LENTH))  
				{
					CommMotDrv.u8_RXCommand = U8_Uart4_RxBuf[3];                
								   
					CommMotDrv.RxBuf.u8_Byte5  = U8_Uart4_RxBuf[5];                //接收数据
					CommMotDrv.RxBuf.u8_Byte6  = U8_Uart4_RxBuf[6];                //接收数据
					CommMotDrv.RxBuf.u8_Byte7  = U8_Uart4_RxBuf[7];                //接收数据
					CommMotDrv.RxBuf.u8_Byte8  = U8_Uart4_RxBuf[8];                //接收数据
					CommMotDrv.RxBuf.u8_Byte9  = U8_Uart4_RxBuf[9];                //接收数据
					CommMotDrv.RxBuf.u8_Byte10 = U8_Uart4_RxBuf[10];               //接收数据
					CommMotDrv.RxBuf.u8_Byte11 = U8_Uart4_RxBuf[11];               //接收数据
					CommMotDrv.RxBuf.u8_Byte12 = U8_Uart4_RxBuf[12];               //接收数据
					CommMotDrv.RxBuf.u8_Byte13 = U8_Uart4_RxBuf[13];               //接收数据
					CommMotDrv.RxBuf.u8_Byte14 = U8_Uart4_RxBuf[14];               //接收数据
					CommMotDrv.RxBuf.u8_Byte15 = U8_Uart4_RxBuf[15];               //接收数据
					CommMotDrv.RxBuf.u8_Byte16 = U8_Uart4_RxBuf[16];               //接收数据
					CommMotDrv.RxBuf.u8_Byte17 = U8_Uart4_RxBuf[17];               //接收数据
				}
...
//        Uart4.u8_TxCnt = 0;								        //通信超时强制发送状态复位(波特率较低不要清0)
    }
