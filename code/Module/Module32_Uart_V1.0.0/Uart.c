...
unsigned char g_tx_OutDoorStatus1;
unsigned char g_tx_OutDoorStatus2;
unsigned char g_tx_OutDoorStatus3;
...
void Uart3_Interrupt_lhh(void) 
{
     if ((UART_GetITStatus(UART2, UART_IT_TB) != RESET)
            && (UART_GetFlagStatus(UART2, UART_FLAG_TB) != RESET))//??????
    {
    ...
    if ((UART_GetITStatus(UART2, UART_IT_RB) != RESET)
            && (UART_GetFlagStatus(UART2, UART_FLAG_RB) != RESET))//??????
    {
...
void Uart0_Interrupt(void) 
{
    ...
    if ((UART_GetITStatus(UART0, UART_IT_RB) != RESET) && (UART_GetFlagStatus(UART0, UART_FLAG_RB) != RESET))         //接收标志位判断
    {
        ...
        {			
            if ((Uart0.u8_RxBuffLast == C_UART0_RX_HEAD_FIRST) && (Uart0.u8_RxBuff == C_UART0_RX_HEAD_SECOND))
            {
                ...
                U8_Uart0_DrvRxBuf[UART0_RX_HEAD_FIRST]  = Uart0.u8_RxBuffLast;
                U8_Uart0_DrvRxBuf[UART0_RX_HEAD_SECOND] = Uart0.u8_RxBuff;
...
void Uart1_Interrupt(void) 
{
    ...
    {
        if (Uart1.u8_TxCnt < UART1_TX_LEN)                          //发送电源板信息12个字节
        {			
...
void    Uart1_SendPackDeal(void)
{
    if (F_uart1_TxReady)                            //串口发送空闲时
    {
		...
		UART_ITConfig(UART1, UART_IT_TB, ENABLE);    //发送有效
...
//		uart_receive_input(dat);
        ...
        {			
            if ((Uart2.u8_RxBuffLast == C_UART2_RX_HEAD_FIRST) && (Uart2.u8_RxBuff == C_UART2_RX_HEAD_SECOND))
            {
            ...
            else if (Uart2.u8_RxCnt >= 2) 
            {
                ...
                if (Uart2.u8_RxCnt >= (U8_Uart2_DrvRxBuf[4]+6))//UART2_RX_LEN)                  //接收信息字节长度(根据功能需要接收多少个字节,可能是常数可能是变量)
                {
...
void    Uart2_SendPackDeal(void)
{
    ...
    if (Uart2.Enum_TxState >= ENUM_TXSTATE_OK)      //状态发送完毕后自动转为发送空闲
    {
...
void Uart4_Interrupt(void) 
{
    ...
    if ((UART_GetITStatus(UART4, UART_IT_RB) != RESET) && (UART_GetFlagStatus(UART4, UART_FLAG_RB) != RESET))         //接收标志位判断
    {
        ...
        {			
            if (Uart4.u8_RxCnt == 0)
			{
				if (Uart4.u8_RxBuff == C_UART4_RX_HEAD_FIRST)
				{
					...
					U8_Uart4_DrvRxBuf[UART4_RX_HEAD_FIRST] = Uart4.u8_RxBuff;
