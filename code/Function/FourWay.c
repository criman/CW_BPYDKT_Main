...
void    Log_FourWay(void)
{
    ...
    else
    {
        ...
        {
            if ((FourWay.f_AppOn == ON) && (PowEn12V.f_DrvOn == ON) && (PowEnPTCB.f_DrvOn == ON))		
            {
                if (Defrost.f_Enable || ((Comp.u16_RestartDelay <= 50) || ((Comp.u8_SelTestDelay > 0 ) &&(Comp.u8_SelTestDelay <= 50))))
                {
                    //四通阀开启瞬间
...
{
	if (FourWay.f_DrvOn)	P_FourWay_On();
	else					P_FourWay_Off();
}
/****************************************************************************************************
