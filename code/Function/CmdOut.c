...
void    DisplayBoard_Byte7_Update(void)
{
	...
	//检查设定值是否在合理范围内
	if ((Tempr.u8_TempCSet < C_TEMP_TEMPSETMIN) || (Tempr.u8_TempCSet > C_TEMP_TEMPSETMAX))	//非法范围
	{
...
void    DisplayBoard_Byte11_Update(void)
{
	...
	if (System.u8_PowerOn_Count >= 10)	//上电1s后才判断
	{
		...
		else if (F_uart0_CommErr)
		{
			CommDisp.Tx.Byte11.ErrInfo = ENUM_ERRINFO_E8_COMMDISP;		//通讯故障
		}		
		else if (F_uart4_CommErr || System.f_InOutCommErr)
		{		
...
//	//--------------------------------------------------------------------------------------------------//
	...
	if (SystemMode.f_Silence == 1)		CommDisp.Tx.Byte18.var.Silence = Enum_ENABLE;
...
void    DisplayBoard_Byte27_Update(void)
{
	...
	if (SystemMode.f_DeNoise)	 CommDisp.Tx.Byte27.var.DeNoise = Enum_ENABLE;
