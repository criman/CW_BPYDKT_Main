...
void	 App_Status_Off(void)
{
	...
	if (Comp.u32_StopContCount >= 30)		//压缩机停机后3秒
	{
	...
	switch (System.Mode)
	{
		...
		{		
			...
			if (SystemMode.f_Cold == 0)
			{
				if (T1.f_Error == 0)
				{
					if ((T1.s16_ValueMul10bc + 0) <= TempValueMul10(Tempr.u8_TempCSet))
					{
			...
			else if ((T1.f_Error == 1) || (T2.f_Error == 1) || (T3.f_Error == 1) || (T4.f_Error == 1) || (Tp.f_Error == 1))	//传感器故障
			{
		...
		case	ENUM_SYSMODE_HUM:	     //除湿
		{
			...
			if ((T1.f_Error == 1) || (T2.f_Error == 1) || (T3.f_Error == 1) || (T4.f_Error == 1) || (Tp.f_Error == 1))	//传感器故障
			{
				Comp.u16_RunMinDelay = 0;		//故障清0
			...
			{
				if (T1.s16_ValueMul10 >= (TempValueMul10(C_TEMP_TEMPSETMIN) +20))		//T1≥18℃ 
				{				
					Comp.f_AppOn = ON;
					SystemMode.f_TempCEC = 0;
					Protect.f_HumiT1Low = 0;
				}
				else if (T1.s16_ValueMul10 <= TempValueMul10(C_TEMP_TEMPSETMIN))	//T1≤16℃ 
				{
					Comp.f_AppOn = OFF;
		...
		case	ENUM_SYSMODE_FAN:        //送风
		{
			...
			Fan.Outdoor.u16_Delay = 0;	 //外风机立即停止 延时清0
...
//							}							
		...
		case	ENUM_SYSMODE_HEAT:       //制热
		{
			...
			else if ((T1.f_Error == 1) || (T2.f_Error == 1) || (T3.f_Error == 1) || (T4.f_Error == 1) || (Tp.f_Error == 1))
			{
				Comp.f_AppOn = OFF;
				Comp.u16_RunMinDelay = 0;		//故障清0
			}
...
//						{
//							if (Tempr.u8_TempCSet > C_TEMP_TEMPSETMIN)
//							{
//								Tempr.u8_TempCSet -= 1;
//							}
//						}
...
//							{	
//								if (Tempr.u8_HumiCSet < C_TEMP_TEMPSETMAX)
//								{
//									Tempr.u8_HumiCSet += 1;
//								}
//							}
//							else
//							{
//								if (Tempr.u8_TempCSet < C_TEMP_TEMPSETMAX)
//								{
//									Tempr.u8_TempCSet += 1;
//								}
//							}							
