...
//			SystemMode.f_Force = 0;//强力模式关闭
		...
		if (CmdInBak.Byte5.var.Mode != CommDisp.Rx.Byte5.var.Mode)
		{
			...
			if (System.Mode == ENUM_SYSMODE_HUM)
			{
				...
				if (T1.s8_Value < (C_TEMP_TEMPSETMAX + 2))
				{				
...
//			}
		...
		{
			if (CommDisp.Rx.Byte11.var.Silence == ENUM_ENABLE)			
			{
				SystemMode.f_Silence = 1;//静音开启
				SystemMode.f_Force = 0;//强力模式关闭
			}
			else														
			{
				SystemMode.f_Silence = 0;//静音关闭
			}
...
void    DisplayBoard_Byte15_Con(void)
{
	if (CmdInBak.Byte15.Byte != CommDisp.Rx.Byte15.Byte)				//第15字节发生变化
	{
		...
		{
			if (CommDisp.Rx.Byte15.var.ForceMode == Enum_ENABLE)		
			{
				SystemMode.f_Force = 1;//强力模式开启
				SystemMode.f_Silence = 0;
			}
			else
			{
				SystemMode.f_Force = 0;//强力模式关闭
		...
		if (CmdInBak.Byte15.var.f_DeNoise != CommDisp.Rx.Byte15.var.f_DeNoise)
		{
			if (CommDisp.Rx.Byte15.var.f_DeNoise == Enum_ENABLE)		
			{
				SystemMode.f_DeNoise = 1;
			}
			else 
			{
				SystemMode.f_DeNoise = 0;	
			}
		}
...
//	SystemMode.f_Force = 0;					//关机退出强力模式
	SystemMode.f_AntiDewMark = 0;			//关机清除防凝露不能重复进入标志	（屏蔽此无需关机即可有下一次）
