...
STRUCT_MBTXBYTE10    BaseRunTx_Byte10_InDoorProtect1(void)
{
	...
	temp.f_CommDisp    = F_uart0_CommErr;		   	//室内板与显示板通信故障 E8
...
//	else						temp.f_T4Limit = 0;
	...
	if (SystemMode.f_DeNoise)		temp.f_DeNoise = 1;
	else							temp.f_DeNoise = 0;
	...
	//静音标志
	if ((SystemMode.f_Silence == 1) 
	&& (Comp.u8_RealHZ <= (Comp.u8_Cold_TargetHZ[4] + 1)))
	{
		temp.f_Silence = 1;
	}
	else							
...
void    CommMotDrv_BaseRunRx(void)
{
	...
	{
		g_tx_OutDoorStatus1 = CommMotDrv.BaseRun.Rx.Byte11.Byte;
		//Bit0：电流限频L3
	...
	{
		g_tx_OutDoorStatus2 = CommMotDrv.BaseRun.Rx.Byte12.Byte;
		//Bit0: PFC模块开关停机
...
//		}
		...
		g_tx_OutDoorStatus3 = CommMotDrv.BaseRun.Rx.Byte13.Byte;
		//Bit1：压缩机过流P41
	...
	Fan.Outdoor.u16_TargetRPM = (uint16_t)(CommMotDrv.BaseRun.Rx.Byte14.Byte) <<3;
	//当前室外机的状态4	
	//if (CommMotDrv.BaseRun.Rx.Byte14.Byte)
	{
		...
		//Bit4: 压缩机预热标志
		if (CommMotDrv.BaseRun.Rx.Byte14.OutDoorStatus4.f_CompPreHeat)
		{
...
//	}	
	//IPM模块温度 真实值*2
	System.u8_IPMTemp = CommMotDrv.BaseRun.Rx.Byte15.Byte >> 1;
...
void    CommMotDrv_ReqSetRx(void)
{
	...
	OutDoorFanSpeed <<= 3;
//	Fan.Outdoor.u16_TargetRPM = OutDoorFanSpeed;
	//--------------------------------------------------------------------------------------------------//
