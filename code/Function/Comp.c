...
/****************************************************************************************************
Function Name       :void    App_Comp_Silence(void)
Description         :静音模式下的压缩机频率控制
Input               :
Return              :
Author              :wuhaiming
Version             :V1.0
...
//		App_Comp_ForceFun();		//强力模式下的压缩机频率控制	 				客户需求不对频率修改
	...
	{
		if ((Protect.TpStatus == ENUM_PROSTATUS_FASTDOWN) 
		...
		{		
			if ((Protect.SystemStatus == ENUM_PROSTATUS_NORMAL) 
		...
		|| (Tempr.T2HeatZone == ENUM_TEMPT2HEATZONE_SLOWDOWN))	//存在频率慢速降频的情况		
		{
			...
			if ((Protect.TpStatus == ENUM_PROSTATUS_SLOWDOWN) 
			|| (Protect.T2ColdStatus == ENUM_PROSTATUS_SLOWDOWN) 
			|| (Protect.T3ColdStatus == ENUM_PROSTATUS_SLOWDOWN))
			{
		...
		{
			if ((Protect.TpStatus == ENUM_PROSTATUS_SLOWUP) 
			|| (Protect.T2ColdStatus == ENUM_PROSTATUS_SLOWUP) 
			|| (Protect.T3ColdStatus == ENUM_PROSTATUS_SLOWUP) 
			|| (Tempr.T2HeatZone == ENUM_TEMPT2HEATZONE_SLOWUP))
			{
				if (++Protect.u16_CompHzLimitCount >= 3000)		//30s
				{
					Protect.u16_CompHzLimitCount = 0;
...
void    Log_Comp(void)
{
	if (Comp.f_DrvOn == OFF)
	{
		if (((Protect.T2ColdStatus != ENUM_PROSTATUS_PROTECT) 
		&& (Tempr.T2HeatZone != ENUM_TEMPT2HEATZONE_PRO) 
		...
		&& (Tempr.T4CoolZone != ENUM_TEMPT4COOLZONE_PRO1) 
 		...
 		&& (Protect.CompCurrStatus != ENUM_PROSTATUS_PROTECT) 
 		&& (System.f_T4LimitOff == 0) 
 		&& (Comp.f_IPMPro == 0) 
 		&& (Comp.f_IPMErr == 0) 
 		&& (Comp.f_StartErr == 0) 
		...
		{		
			if ((SystemMode.f_Heat == 1) 
			&& (Comp.f_AppOn == ON) 
			&& (FourWay.f_DrvOn == OFF)) //制热压缩机开启前先保证四通阀打开
			{
			...
			&& (WaterBox.f_Full == 0))
			{
				...
				Comp.u16_RunMinDelay = C_COMP_RUNMINDELAY;			//压缩机最小运行时间3分钟
				WPMotor.u16_CompOnDelay = 900;						//打水电机延时压缩机启动后90s启动
				//Defrost.u16_FindT30_Count = 0;					
				//化霜功能制热时重新计时寻找T30时间 lcx del 20240715 开关机不清除T30时间，
	...
	{
		if (((Comp.f_AppOn == OFF) && (Comp.u16_RunMinDelay == 0)) 
		|| WaterBox.f_Full 
		...
		|| (Protect.CurrHeatStatus == ENUM_PROSTATUS_PROTECT) 
 		|| (Protect.CompCurrStatus == ENUM_PROSTATUS_PROTECT)) 
 		|| System.f_T4LimitOff 
 		|| Comp.f_IPMPro || Comp.f_IPMErr || Comp.f_StartErr 
 		|| System.f_CurrentProtect || System.f_VoltLowPro 
 		|| Fan.Indoor.f_Error || Fan.Outdoor.f_Error) 
 		&& (System.Enum_Status != ENUM_STATUS_BUSSCHK)))	
		{
			...
			if (System.Enum_Status != ENUM_STATUS_BUSSCHK)
			{
				if ((T4.f_Error == 0) 
				&& (Tp.f_Error == 0) 
				&& (T4.s16_ValueMul10 > 500) 
				&& (Tp.s16_ValueMul10 > 700))		//室外温度超50℃且排气温度超70℃
				{
			...
			if (SystemMode.f_HeatBak == 1)
			{
				if (T2.s16_ValueMul10 > 300)	
				{
					...
					if (T2.s16_ValueMul10 < 350)	//30℃＜IPT＜35℃
					{
			...
			{
				if (Comp.u32_RunContCount == 0)			//避免压缩机停机超过3s时外风机比压机先停的情况
				{
					Fan.Indoor.u16_Delay = 30;			//内风机延时3秒关闭
					Fan.Outdoor.u16_Delay = 30;			//外风机延时3s后关闭
				}
			}
			
			if (System.Enum_Status != ENUM_STATUS_BUSSCHK)
			{
				FourWay.u16_Delay = 1200;			//压缩机关机2分钟后关闭四通阀(除霜除外)
			}
			else
			{
...
{	
	...
	if (Comp.f_DrvOn == 1)
	{
		if (System.Enum_Status != ENUM_STATUS_BUSSCHK)
		{
			...
			{
				if ((SystemMode.f_Cold == 1) 
				&& ((Tempr.u8_TempCSet == 30) 
				|| (Tempr.u8_TempCSet == 29)))	//额定制冷/最小制冷
				{
					Comp.u8_TargetFreq = Comp.u8_TargetHZ;
				}
