...
void	App_TCLModule_Slave_Info(void) //主控发送给WIFI
{
	...
	//第4字节
	//设定温度
	if ((Tempr.u8_TempCSet >= C_TEMP_TEMPSETMIN) && (Tempr.u8_TempCSet <= C_TEMP_TEMPSETMAX))
	{
		WiFi.Slave.Byte4.SlaveByte4.TempSet = (ENUM_WIFITEMPSET)(C_TEMP_TEMPSETMAX - Tempr.u8_TempCSet);
	}
	else
	{
...
//		else						WiFi.Slave.Byte4.SlaveByte4.WindSpeed = ENUM_WIFIWINDSPEED_LOW;			//低速
	...
	if (F_uart0_CommErr)//(CommDisp.Tx.Byte11.ErrInfo == ENUM_ERRINFO_E8_COMMDISP)
	{
	...
	if (F_uart4_CommErr || System.f_InOutCommErr)//(CommDisp.Tx.Byte11.ErrInfo == ENUM_ERRINFO_EF_INOUTCOMMERR)
	{
	...
	if (SystemMode.f_Sleep)
	{
		...
		{
			u8_temp += 1;
			if (u8_temp > C_TEMP_TEMPSETMAX)	u8_temp = C_TEMP_TEMPSETMAX;
			
		}		
		else if (SystemMode.f_Heat == 1)
		{
			u8_temp -= 1;
			if (u8_temp < C_TEMP_TEMPSETMIN)	u8_temp = C_TEMP_TEMPSETMIN;		
		}
...
//		{		
			...
			if (temp)
			{
				...
				{
					//进入除湿模式计算设定温度:    ST = T1 - 2
					if (T1.s8_Value < (C_TEMP_TEMPSETMIN + 2))
					{				
						Tempr.u8_HumiCSet = C_TEMP_TEMPSETMIN;	//最低
					}
					else if (T1.s8_Value > (C_TEMP_TEMPSETMAX + 2))
					{
						Tempr.u8_HumiCSet = C_TEMP_TEMPSETMAX;	//最高
					}
					else
					{
						Tempr.u8_HumiCSet = T1.s8_Value - 2;
...
//		{
			temp = C_TEMP_TEMPSETMAX - WiFi.Master.Byte5.MasterByte5.TempSet;
			...
			Tempr.u8_TempCSet = C_TEMP_TEMPSETMAX - WiFi.Master.Byte5.MasterByte5.TempSet;
			if (Tempr.u8_TempCSet > C_TEMP_TEMPSETMAX)		Tempr.u8_TempCSet = C_TEMP_TEMPSETMAX;
			if (Tempr.u8_TempCSet < C_TEMP_TEMPSETMIN)		Tempr.u8_TempCSet = C_TEMP_TEMPSETMIN;
...
U8	App_TCLModule_CtoF(U8 tempcset, U8 Fadd1)
{
	...
	if (tempcset < C_TEMP_TEMPSETMIN)	tempcset = C_TEMP_TEMPSETMIN;	//min
	if (tempcset > C_TEMP_TEMPSETMAX)	tempcset = C_TEMP_TEMPSETMAX;	//max
